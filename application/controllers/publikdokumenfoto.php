<?php
class Publikdokumenfoto extends CI_Controller{
   ...
   $config['base_url'] = base_url().'publikdokumen/index';
   ...
   $data['active_dokumenktp'] = 'active';
   $data['kd_file']= $this->m_app->getKodeFile();
   //$data['data_anggota'] = $this->m_app->getAllData('tbl_anggota');
   $data['ambil_imgfoto'] = $this->m_app->ambil_imgfoto($config['per_page'], $offset);
  
        $this->load->view('publik/v_dokumenfoto',$data);
...
public function add_foto(){
        ...
        $config['allowed_types'] = 'pdf|gif|jpg|png|jpeg|bmp'; //type yang dapat diakses bisa anda sesuaikan
        ...
        if($_FILES['filefoto']['name'])
        {
            if ($this->upload->do_upload('filefoto'))
            {
                ...
                redirect('publikdokumenfoto'); //jika berhasil maka akan ditampilkan view vupload
            }else{
                ...
                redirect('publikdokumenfoto'); //jika gagal maka akan ditampilkan form upload
            }
          }
} 
